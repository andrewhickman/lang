...
impl<'src> fmt::Display for Token<'src> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            Ident(ident) => return write!(f, "identifier: '{}'", ident),
            Num(num) => return write!(f, "integer literal: '{}'", num),
            Invalid(ch) => return write!(f, "invalid character: '{}'", ch),
            _ => (),
        }
        write!(f, "token: '{}'", match *self {
            Plus => "+",
            PlusPlus => "++",
            Minus => "-",
            MinusMinus => "--",
